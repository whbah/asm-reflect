...
import com.fisher.core.constraint.MethodInvokeReturnType;
import com.fisher.core.util.reflect.MethodUtil;
...
public class AsmByteBuilder extends ClassLoader implements Opcodes {
	...
	private MethodVisitor handleReturn(MethodVisitor mw, int returnType) {
		if (returnType == MethodInvokeReturnType.VOID) {
			mw.visitInsn(RETURN);
		} else {

			switch (returnType) {
			case MethodInvokeReturnType.DRETURN:
				mw.visitMethodInsn(INVOKESTATIC, mDoubleClassName,
						mDoubleValueOfMethodName, mDoubleValueOfMethodDes,
						false);
				break;
			case MethodInvokeReturnType.FRETURN:
				mw.visitMethodInsn(INVOKESTATIC, mFloatClassName,
						mFloatValueOfMethodName, mFloatValueOfMethodDes, false);
				break;
			case MethodInvokeReturnType.IRETURN:
				mw.visitMethodInsn(INVOKESTATIC, mIntegerClassName,
						mIntegerValueOfMethodName, mIntegerValueOfMethodDes,
						false);
				break;
			case MethodInvokeReturnType.LRETURN:
				mw.visitMethodInsn(INVOKESTATIC, mLongClassName,
						mLongValueOfMethodName, mLongValueOfMethodDes, false);
				break;
			case MethodInvokeReturnType.ARETURN:
				break;
			}
